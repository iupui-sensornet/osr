 ...
 * With snoop children 
 *
 * This version use adaptive max number of multicasts. In default version, multicast 
 * would be transmitted for OSR_MCAST_RETRIES. In current version, use the unicast PRR
 * to define the max number of multicast transmissions.
 
 * The same for broadcast
 */
...
//		interface Intercept[osr_id_t id];
	...
	uses {
		...
		// Snoop Children using Ctp Packets 
		interface Receive as CtpSnoop[collection_id_t id];
...
implementation {
	...
	// children table extension, use the last 5 locations for snooped children 
	// that are not in the direct children set 
	// 		the total table size is unchanged, or can be extended 
	children_table_entry_t children[OSR_CHILDREN_SIZE + SNOOP_SIZE];
	...
	//// for adaptive number of multicasts
	//// adaptive only if ucast_tx > 10 (there are 10 ucast_tx)
	//// max_mcast_retries = 2 * (1/link prr) link prr = 1 - ucast_retx / ucast_tx
	////	if max_mcast_retries > OSR_MCAST_RETRIES, max_mcast_retries = OSR_MCAST_RETRIES
	uint16_t ucast_tx = 0;	
	uint16_t ucast_retx = 0;
//	uint8_t max_mcast_retries = OSR_MCAST_RETRIES;	// also for broadcast
	
	// return the max number of multicast retx
	int8_t compute_max_retx() {
		int8_t max_mcast_retries = OSR_MCAST_RETRIES;
		if(ucast_tx >= 10) {
			// calculate PRR only after 10 total unicasts
			// the number 250 UL comes from 4bitLE
			uint8_t link_prr = 250UL - (uint16_t)ucast_retx*250UL/ucast_tx;
			int8_t tmp =(int8_t) (2500UL / link_prr);	// tmp >= 10,
			if(tmp < 15) {	// round up the computed value
				tmp = 1;
			} else if(tmp < 25) {
				tmp = 2;
			} else if (tmp < 35) {
			...
			max_mcast_retries = 1 + tmp;
			if (max_mcast_retries > OSR_MCAST_RETRIES) {
				max_mcast_retries = OSR_MCAST_RETRIES;
			}
		} 
		return max_mcast_retries;
	}
	...
	command error_t Init.init() {
		...
		for(i = 0; i < OSR_CHILDREN_SIZE + SNOOP_SIZE; i++) {
			children[i].id = 0;
			children[i].ttl = 0;
		}
...
//		call SubPacket.setPayloadLength(msg, len + sizeof(osr_header_t));
		...
		if(qe->cur_next_hop_idx > MCAST_THRD){
			...
			/////// for adaptive multicast 
			qe->retries = compute_max_retx();
...
//			qe->retries = OSR_BCAST_RETRIES;
	...
	task void sendTask() {
		...
		if (hasState(SENDING) || call SendQueue.empty()) {
			...
			osr_tail_t* tail = getTail(qe->msg, call Packet.payloadLength(qe->msg));
...
//			uint8_t payloadLen = call SubPacket.payloadLength(qe->msg);
			...
			if(TOS_NODE_ID != SINK_ADDR && is_fp_node(hdr->down_path, TOS_NODE_ID)) {
				dbg("TestFP", "seqno: %d, TestFP: Transmission caused by False positive node (nodes not in the path): %d\n", hdr->seqno, TOS_NODE_ID);
				call OsrInstrumentation.hlth_fp_tx();	// Transmissions caused by nodes not in the path, not necessary to be false positive caused by false positive
				
				
				for(i = 0; i < MAX_BFLT_LEN; i++){
					my_bflt[i] = 0;
				}
				call BloomFilter.getBflt(TOS_NODE_ID, my_bflt, hdr->path_len);
		
				if(bfltMatched(my_bflt, tail->path_bflt)) {
					dbg("TestFP", "seqno: %d, TestFP: Transmission caused by False positive node (Actual false positive nodes): %d\n", hdr->seqno, TOS_NODE_ID);
//					call OsrInstrumentation.hlth_fp_tx_fp();
				}
			}
			...
			if(hdr->option == OSR_UCAST) {
				...
				////////// for adaptive multicast 
					...
					printf("-------- The max number of bcast/mcast is: %d\n", compute_max_retx());
					printfflush();
				#endif
...
//						qe->retries = OSR_BCAST_RETRIES;
						...
						// if the packet is received by broadcast, do not do more OPP
						call SendQueue.dequeue();
						clearState(SENDING);
						call MessagePool.put(qe->msg);
						call QEntryPool.put(qe);
...
//									OSR_MCAST_OFFSET * (OSR_MCAST_RETRIES - qe->retries));
					...
					startRetxTimer(OSR_MCAST_WINDOW * (compute_max_retx() - qe->retries), 
									OSR_MCAST_OFFSET * (compute_max_retx() - qe->retries));
...
//					startRetxTimer(OSR_SENDDONE_OK_WINDOW, OSR_SENDDONE_OK_OFFSET);
	...
	message_t* ONE forward(message_t* ONE m) {
		if(call MessagePool.empty()) {
			...
			for(i = 0; i < OSR_CHILDREN_SIZE + SNOOP_SIZE; i++) {	// add snoop children 
				...
				if(children[i].id != 0) {
					...
					if(bfltMatched(chd_bflt, tail->path_bflt)) {
						...
						// for Snoop children 
						// don't do this for the Sink, for sink always has direct children.
						if(TOS_NODE_ID != SINK_ADDR && i >= OSR_CHILDREN_SIZE) {
							// this is a children added by snoop CTP packets 
							dbg("SnoopChildren", "seqno: %d, SnoopChild: matched snooped children: %d\n", hdr->seqno, children[i].id);
						}
					}
			...
			// Debug FP
			// for opportunistic routing (broadcast), check whether it is delivered to a false positive child
			// The node itself does not matter
			...
			if(qe->rcv_option == OSR_BCAST && qe->cur_next_hop_idx>0) {
				...
				for(i = 0; i < qe->cur_next_hop_idx; i++) {
					if(is_fp_node(hdr->down_path, qe->next_hops[i])) {
						if(fp_counted == 0) {
							/// only count once
							dbg("TestFP", "seqno: %d, TestFP: OPP (Bcast_rcv)acted to False Positive child: %d . False bypassing\n", hdr->seqno, qe->next_hops[i]);
							call OsrInstrumentation.hlth_fp_opp();	
							fp_counted = 1;
//							break;
						}
					} else {
						// true node, opp delivers to true node 
						if(tp_counted == 0) {
							dbg("OSR_Receive", "seqno: %hu, (Bcast_rcv)act opportunistic routing, pkt received from broadcast. \n", 
										hdr->seqno);
			...
			// Debug FP
			// OPP routing from multicast, this node must be false positive 
			if(qe->rcv_option == OSR_MCAST && qe->cur_next_hop_idx>0) {
				int fp_counted = 0;
				int tp_counted = 0;
				//// if TOS_NODE_ID not in the path, then this node is a false positive, and this is opportunistic routing 
				//// if the node is in the path, then it is not opportunistic routing 
				if(is_fp_node(hdr->down_path, TOS_NODE_ID)) {
					...
					for(i = 0; i < qe->cur_next_hop_idx; i++) {
						if(is_fp_node(hdr->down_path, qe->next_hops[i])) {
							if(fp_counted == 0) {
								/// only count once
								dbg("TestFP", "seqno: %d, TestFP: OPP (Mcast_rcv)acted to False Positive child: %d . False bypassing\n", hdr->seqno, qe->next_hops[i]);
//								call OsrInstrumentation.hlth_fp_opp();	// not count for multicast
								fp_counted = 1;
	//							break;
							}
						} else {
							// true node, opp delivers to true node 
							if(tp_counted == 0) {
								dbg("OSR_Receive", "seqno: %hu, (Mcast_rcv)act opportunistic routing, pkt received from broadcast. \n", 
											hdr->seqno);
								call OsrInstrumentation.hlth_byp_acted();
								tp_counted = 1;
							}
...
//		
	...
	/******************** Snoop CTP packets to add snoop children ***********************/
	void snoop_update(uint16_t child){
		bool found = FALSE;
		int i;
		for(i = OSR_CHILDREN_SIZE; i < OSR_CHILDREN_SIZE + SNOOP_SIZE; i++) {
...
//					dbg("SnoopChildren","SnoopChildren ------ Add child: %hu\n", child);
				}
				else {
					// cannot be put in the table, print error 
					dbg("OsrError", "OsrError: snoop table full (size: %d),  cannot add child: %d\n", i, child);
				}
				...
				child_idx++;
//				if(child_idx >= OSR_CHILDREN_SIZE + SNOOP_SIZE) {
//					child_idx = OSR_CHILDREN_SIZE;
//				}
				
				dbg("SnoopChildren", "SnoopChildren snooped children count: %d\n", child_idx-OSR_CHILDREN_SIZE);
//				child_idx %= OSR_CHILDREN_SIZE;
//				children_count++;
//				dbg("SnoopChildren","SnoopChildren ------ Add child: %hu\n", child);
			}
	...
	event message_t* CtpSnoop.receive[collection_id_t collectionid](message_t* msg, void* payload, uint8_t len) {
		// get the snooped packet path ETX
		// compare with myself's path ETX
		// if rcv_path_etx - my_path_etx > 10 (one transmission), add it as direct children 
		uint16_t my_path_etx;
		uint16_t rcv_path_etx;
		uint16_t child = call AMPacket.source(msg);
		
//		dbg("SnoopChildren", "SnoopChildren: snooped CTP packet from %d\n", child);
		rcv_path_etx = call CtpPacket.getEtx(msg);
		call CtpInfo.getEtx(&my_path_etx);
		
		if (rcv_path_etx - my_path_etx > 10) {
//			dbg("SnoopChildren", "SnoopChildren: The node %d can be added to the snoop table \n", child);
			snoop_update(child);
		}
		
		return msg;
	}
	/****************************** end of snoop childrne *****************************/
...
//		bflt = hdr->path_bflt;
	...
	#if defined(PRINTF_INDRIYA_ENABLED) 
	command void OsrPacket.setDownPath(message_t* msg, uint8_t* down_path) { // For Indriya test to same RAM
	#else
	...
	command void ChildrenTable.update(uint16_t child) {
		...
		if(!found) {
			...
			if (table_full) {
				...
				child_idx %= OSR_CHILDREN_SIZE;
				children_count++;
				dbg("ChildrenCount","------ Children Add child: %hu, children_count: %d\n", child, children_count);
			}
