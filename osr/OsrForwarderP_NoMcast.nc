...
//					call OsrInstrumentation.hlth_fp_tx_fp();
			...
			if (subsendResult == SUCCESS) {
				...
				setState(SENDING);
				dbg("OSR_Task", "seqno: %d, OSR_Task: %s: subsend succeeded.\n", hdr->seqno, __FUNCTION__);
				return;
			} 
	...
	void packetComplete(osr_fe_queue_entry_t* qe, message_t* msg, bool success) {
		...
		if (qe->client < CLIENT_COUNT) { 
			...
			if(hdr->option == OSR_BCAST || hdr->option == OSR_MCAST){
				...
				// if there are still packet to sent
				qe->cur_next_hop_idx--;
				if(qe->cur_next_hop_idx > 0) {
					// still target to send
					qe->retries = OSR_UCAST_RETRIES;
					hdr->next_hop = qe->next_hops[qe->cur_next_hop_idx-1];
					...
					if (qe->unicast_ever_failed) {
						// there is a unicast failure, use opportunistic routing 
						//qe->retries = compute_max_retx();
qe->retries = OSR_MCAST_RETRIES;
						...
						// all unicast success, normal return 
