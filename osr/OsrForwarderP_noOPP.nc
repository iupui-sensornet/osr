 ...
 *
 * No Opportunistic routing, 
 	- if a node is not in the path, ignore the packet
 	- if unicast fails, no broadcast 
 *
 * 
 * Without snoop children 
 *
...
//			}
		...
		////////////////////// if the node is not in the source-route, ignore the packet 
		if (is_fp_node(rcv_hdr->down_path, TOS_NODE_ID) && (TOS_NODE_ID != rcv_hdr->target_id)) {
			// this node is not in the downstream path, ignore this node 
			return msg;
		}
