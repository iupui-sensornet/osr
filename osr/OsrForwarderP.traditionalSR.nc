...
//			dbg("TestMatch", "TestMatch: j: %d, Bflt1: %d, Bflt2: %d\n", j, bflt1[j], bflt2[j]);
	...
	command error_t Send.send[uint8_t client](message_t* msg, uint8_t len){
		...
		hdr->ttl = call OsrPacket.getPathLen(msg);	// path: [target, id1, ..., sink] path length = size(path_array)-1
...
//		call SubPacket.setPayloadLength(msg, len + sizeof(osr_header_t));
		...
		// DON't use the children table to find the next hop, instead, using the path directly 	
//		for(i = 0; i < OSR_CHILDREN_SIZE; i++) {
//			if(children[i].id != 0) {
//				int j;
//				for(j = 0; j < MAX_BFLT_LEN; j++) {
...
//		}
		...
		qe->next_hops[0] = hdr->down_path[hdr->ttl-1];
		qe->cur_next_hop_idx = 1;
...
// for traditional source routing
			qe->next_hops[0] = hdr->down_path[hdr->ttl-1];
			qe->cur_next_hop_idx = 1;
